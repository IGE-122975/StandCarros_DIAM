...
def vehicle_pdf(request, pk):
    ...
    def link_callback(uri, rel):
        if uri.startswith(settings.MEDIA_URL):
            # Constrói o caminho e garante que está dentro de MEDIA_ROOT
            # para evitar path traversal (ex: URI com ".." no caminho)
            relative = uri.replace(settings.MEDIA_URL, '', 1).lstrip('/')
            path = os.path.normpath(os.path.join(settings.MEDIA_ROOT, relative))
            if path.startswith(os.path.normpath(settings.MEDIA_ROOT)) and os.path.exists(path):
                return path
        return uri
